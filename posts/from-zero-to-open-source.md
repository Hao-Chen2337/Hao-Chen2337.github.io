...
一看到乱码，我就懂了：慢可能是性能有优化空间，乱码那基本就是 bug 了。

顺着链路排查下去，最后定位到SpaceMiT 后端的派发表。派发表里其余量化类型都有分支，唯独缺了 `GGML_TYPE_Q5_0`。`Q4_K_M` 属于混合量化格式，内部刚好包含 `Q5_0` 分片，所以才触发了缺失分支。

这里说的派发表（dispatch table），说白了就是一张「量化类型 → 对应计算函数」的查表。程序拿到一个类型，不用写一长串 if/else 逐个判断，直接拿类型当索引查这张表，跳到对应的实现——这是运行时里很常见的「按值分发」手法（维基百科的 [Dispatch table](https://en.wikipedia.org/wiki/Dispatch_table) 词条讲的就是这个）。

```cpp
case GGML_TYPE_Q5_0:
```

碰巧的是，这个格式后面的计算逻辑其实已经写好了，只差这一句把它接上。说来挺奇妙的——这么大的项目，我一个刚进来的新人，居然靠一行代码就能做出点实在的贡献，就好像前面的一切早就安排好了，就等着有人来把这一句补上。
